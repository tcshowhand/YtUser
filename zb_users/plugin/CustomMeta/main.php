...
if(count($_POST) > 0){
    ...
    if(isset($_POST['meta_cate'][$key])){
        $single_meta_cate = $_POST['meta_cate'][$key];
        $zbp->Config('CustomMeta')->$name_meta_cate = $single_meta_cate;
    }
...
<?php
        ...
        foreach ($array as $key => $value) {
            ...
            $single_meta_cate = $type . '_' . $value . '_cate';
            if(!$single_meta_type){
            ...
            $single_meta_cate = $type . '_' . $value . '_cate';
            $single_meta_cate = $zbp->Config('CustomMeta')->$single_meta_cate;
            $distr = 'display:none';
            ...
            if($type=='post'){
            echo '<p>所属分类'.$single_meta_cate.'：<select style="width:85%" name="meta_cate[]">';
            echo OutputOptionItemsOfCategories($single_meta_cate);
            echo '</select></p>';
            }
            echo '</td>';
            echo '<td><p>';
