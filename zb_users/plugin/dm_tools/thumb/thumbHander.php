...
class dm_tools_thumb_ThumbHandler{
    var $dst_img;// 目标文件
    var $h_src; // 图片资源句柄
    var $h_dst;// 新图句柄
    var $h_mask;// 水印句柄
    ...
    // 文件类型定义,并指出了输出图片的函数
    var $all_type = array(
        "jpg"  => array("output"=>"imagejpeg","type"=>"image/jpg"),
        "gif"  => array("output"=>"imagegif","type"=>"image/gif"),
        "png"  => array("output"=>"imagepng","type"=>"image/png"),
        "wbmp" => array("output"=>"image2wbmp","type"=>"image/vnd.wap.wbmp"),
		"bmp" => array("output"=>"image2wbmp","type"=>"image/vnd.wap.wbmp"),
        "jpeg" => array("output"=>"imagejpeg","type"=>"image/jpeg"));
    /**
     * 构造函数
     */
   ...
   /**
    ...
    function _output(){
        ...
        if(function_exists($func_name)){
            // 判断浏览器,若是IE就不发送头
			if(isset($this->dst_img)){
				//if(isset($_SERVER['HTTP_USER_AGENT'])){
				//	$ua = strtoupper($_SERVER['HTTP_USER_AGENT']);
				//	if(!preg_match('/^.*MSIE.*$/i',$ua)){
						//header("Content-typeimg_type");
						header('Content-type: '.$this->all_type[$img_type]['type']);
				//	}
				//}
			}
        ...
        if(2 == $num){
            ...
            switch($this->cut_type){
                ...
                case 2:
                    ...
                    $this->copy_w = $this->src_w;
                    $this->copy_h = $this->src_h;
					if($this->copy_w>$img_w){
						$this->copy_w=$img_w;
					}
					if($this->copy_h>$img_h){
						$this->copy_h=$img_h;
					}
                    // 目标尺寸
                    $this->dst_w   = $this->fill_w + $this->img_border_size*2;
				...
				case 3:
					// 如果原图大于缩略图，产生缩小，否则不缩小
					$rate_w = $img_w/$this->src_w;
					$rate_h = $img_h/$this->src_h;
                    if($rate_w < 1 && $rate_h < 1){
						if($rate_w>$rate_h){
							$this->_setNewImgSize($rate_w*100);
						}else{
							$this->_setNewImgSize($rate_h*100);
						}
					}else{
						$this->_setNewImgSize(100);
					}
					break;
				case 5:
					$this->copy_w = $this->src_w;
    ...
    function _mkdirs($path){
		global $blogpath;
		$path=str_replace(ZBP_PATH,"",$path);
		//echo $path;die();
        $adir = explode('/',$path);
        $dirlist = '';
        ...
        foreach($adir as $key=>$val){
            if($val!='.'&&$val!='..'){
                $dirlist .= "/".$val;
                $dirpath = ZBP_PATH.$dirlist;
				if(dm_tools_thumb::startWith($dirpath,$blogpath)==true){
