 ...
 */
class static_category extends nbseo_batch {
		...
		foreach ($cate_list as $key => $cate) {
			...
			for ($i=1; $i <= $page_num; $i++) {
				...
				if (strpos($zbp->option['ZC_CATEGORY_REGEX'], '{%id%}') !== false) {
					$data[2] = $cate->ID;
				}elseif (strpos($zbp->option['ZC_CATEGORY_REGEX'], '{%alias%}') !== false) {
					$data[2] = $cate->Alias;
				}
				$this->set_queue('static_index_build', json_encode($data));
			}
		...
		foreach ($mem_list as $key => $mem) {
			...
			for ($i=1; $i <= $page_num; $i++) {
				...
				if( $i >1 ) {
					...
					$alisa = ($mem->Alias == '' ? $mem->Name : $mem->Alias);
				...
				if (strpos($zbp->option['ZC_AUTHOR_REGEX'], '{%id%}') !== false) {
					$data[3] = $mem->ID;
				}elseif (strpos($zbp->option['ZC_AUTHOR_REGEX'], '{%alias%}') !== false) {
					$data[3] = $mem->Alias;
				}
				$this->set_queue('static_index_build', json_encode($data));
			}
