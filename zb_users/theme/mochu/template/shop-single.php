...
<div class="container main">
		...
		<div class="shop">
			...
			<div class="shopimg lf">
				<img src="{$article.Metas.mochu_tupian}" alt="{$article.Title}" title="{$article.Title}" />
			</div>
			<div class="shopul lf">
				<ul>
					<li><strong>商品活动：</strong>{$article.Metas.mochu_shiyong}</li>
					<li><strong>商品价格：</strong><span>{$article.Metas.mochu_jiage}</span>&nbsp;元</li>
					<li><strong>已售数量：</strong><span>{$article.Metas.mochu_yishou}</span>&nbsp;套</li>
					<li><strong>发布日期：</strong>{$article.Time('Y-m-d')}&nbsp;&nbsp;(此日期为商品在本站发布的时间)</li>
					<li><strong>商品特点：</strong>{$article.Metas.mochu_tedian}</li>
				</ul>
				<div class="buy">
...
<i class="fa fa-thumbs-o-down"></i><i class="sf-praise-sdk" sfa='num' data-value='2' data-postid='{$article.ID}'>{$sf_praise_sdk.value2}</i>人鄙视
			...
			<div class="nextnav">
				...
				{if $article.Prev}<a href="{$article.Prev.Url}" title="{$article.Prev.Title}">{$article.Prev.Title}</a>{else}博主有点懒,啥也没写!{/if}
				</div>
