...
{if $type=='index' || $zbp->Config('mochu')->onlink=='1'}	
		<div class="footlink">
			<ul>
...
{/if}		
		<div class="footcont">
			<div class="footcont-lf lf">
				<p>博客相关：</p>
				<ul>
					{$zbp->Config('mochu')->postpage}
					<div class="clear"></div>
				</ul>				
				<span>Powered By {$zblogphpabbrhtml} | {$copyright}</span>
			</div>
			<div class="footcont-lr lr">
				<p>欢迎您关注我：</p>
				<div class="footcont-lr-cont">
					<div class="footcont-lr-cont-img lf">
        <img src="{$host}zb_users/cache/mochuimg/erweio.png" alt="关注我的微信二维码" title="关注我的微信二维码" />
					</div>
					<div class="footcont-lr-cont-a lf">
					{$zbp->Config('mochu')->postlian}
					</div>
				</div>
			</div>
			<div class="clear"></div>
...
{if $zbp->Config('mochu')->postguon=="1"}<div id="tbox">
 <a id="pinglun" title="QQ联系我" href="http://wpa.qq.com/msgrd?v=3&amp;uin={$zbp->Config('mochu')->postguqq}&amp;site=qq&amp;menu=yes" target="_blank" rel="nofollow"><i class="fa fa-qq"></i></a> 
 <a id="xiangguan" title="给我留言" href="{$zbp->Config('mochu')->postguping}" target="_blank"><i class="fa fa-comments"></i></a> 
 <a id="gotop" title="返回顶部" href="javascript:void(0)" style="display: inline-block;"><i class="fa fa-arrow-up"></i></a> </div>{/if} 
...
<div id="t-opo"><a href="javascript:void(0)" id="tcaguan" ><i class="fa fa-times"></i></a><div id="t-opo-title"><i class="fa fa-user"></i>  登录</div><div id="t-opo-content">
 ...
 <form method="post" action="#" id="dengform">
      		<h6 class="username"><i class="fa fa-user"></i><input type="text" id="edtUserName" name="edtUserName" size="20" value="<?php echo GetVars('username', 'COOKIE') ?>" tabindex="1" placeholder="用户名"  />		
			</h6>
...
<span class="cp-login"><a href="{$host}zb_system/cmd.php?act=login"></a></span>&nbsp;&nbsp;<span class="cp-vrs"><a href="{$host}zb_system/cmd.php?act=misc&amp;type=vrs"></a></span></div></div></div>
...
<script src="{$host}zb_users/theme/{$theme}/script/mochu_feiniaomy.js" type="text/javascript"></script>
{if $zbp->Config('mochu')->onhuandeng=="1" }
<script src="{$host}zb_users/theme/mochu/script/slides.js" type="text/javascript"></script>
{/if}
{$footer}
{if $zbp->Config('mochu')->ontuisong=="1"}{$zbp->Config('mochu')->posttuisong}{/if}
</body>
</html>