<li class="cate{$article.Category.ID} auth{$article.Author.ID}">
	<div class="timg">
		...
		{/php}
		<a href="{$article.Url}" title="[置顶]{$article.Title}"><img src="{$temp}" alt="{$article.Title}" /></a>
		<div class="tinfo"><a href="{$article.Url}" title="[置顶]{$article.Title}">[置顶]{$article.Title}</a></div>
	</div>
	<div class="tsale">
